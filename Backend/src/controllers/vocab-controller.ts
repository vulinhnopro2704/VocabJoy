...
import { downloadAudio } from "../helpers/download-audio";
import path from "path";

export const createVocabController = async (
	req: Request,
	res: Response,
	next: NextFunction
) => {
...
export const get10VocabController = async (
	req: Request,
	res: Response,
	next: NextFunction
) => {
...
export const getVocabByNameController = async (
	req: Request,
	res: Response,
	next: NextFunction
) => {
...
export const translateToVieController = async (
	req: Request,
	res: Response,
	next: NextFunction
) => {
	try {
		...
		if (!word) throw new HttpException(400, "invalid input");
...
export const addVocab = async (req, res) => {
	try {
		...
		const existingVocab = await Vocab.findOne({
			name,
			pronunciation,
			type,
		});
		...
		//Check if the audio is a link, download it and update the audio property with the filename
		if (newVocab.audio.startsWith("http")) {
			const filename = path.basename(newVocab.audio);
			try {
				await downloadAudio(newVocab.audio, filename);
				newVocab.audio = filename;
		...
		//Save the new vocab to the database
