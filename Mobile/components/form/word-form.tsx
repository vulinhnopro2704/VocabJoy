...
import MaterialIcons from '@expo/vector-icons/MaterialIcons';import { Audio } from 'expo-av';
import { useEffect, useState } from "react";
import { Formik } from "formik";
import { useGetUserIdQuery, useGetVocabularyQuery, useSaveVocabForUserMutation, useSaveVocabularyMutation } from "@/lib/features/api/api-slice";
...
export const Word: React.FC<{ vocab: Vocab }>  = ({vocab})=>{
    ...
    const [saveVocabulary, { error: errorSave, isLoading: loadingSave }] = useSaveVocabularyMutation();
    const { data:vocabFind, error: errorGet, isLoading: loadingGet,refetch } = useGetVocabularyQuery(vocab.name);
    ...
    return (
        <View style={styles.container}>
            <Text style={{fontSize:30,fontWeight:"600"}}>{vocab.name}</Text>
            ...
            <View style={styles.container_mean}>
                <View style={styles.mean}>
                    <Text style={{fontSize:16,fontWeight:"600"}}>{vocab.meaning}</Text>
                ...
                </View>
                {
                    ...
                    <TouchableOpacity onPress={saveVocab}>
                            <View style={styles.button_save}>
                                <MaterialIcons name="data-saver-on" size={14} color="white" />
            ...
            <View style={styles.container_img}>
                <Image style={styles.img} source={require("../../assets/images/mochi.png")}></Image>
            </View>
        </View>
    )
}
